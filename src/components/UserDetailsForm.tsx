...
interface UserDetailsFormProps {
  orderId: string;
  onSubmitSuccess: (userData: { name: string; email: string }) => void;
}
...
const UserDetailsForm = ({ orderId, onSubmitSuccess }: UserDetailsFormProps) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (result.success) {
        ...
        window.history.pushState({ page: 'ebook', userData: { name: formData.name, email: formData.email } }, '', '/ebook');
        
        // Call onSubmitSuccess with user data after 2 seconds
        setTimeout(() => {
          onSubmitSuccess({
            name: formData.name,
            email: formData.email
  ...
  // This is now handled by PaymentFlow - removed to avoid conflicts
